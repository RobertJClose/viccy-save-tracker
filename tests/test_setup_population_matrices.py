"""
Tests for the population matrices setup (setup/research_modifiers/build_population_matrices.py).
...
from setup.research_modifiers import build_population_matrices
from helpers import CAT_SAVE, make_category_game_dir


class TestPopulationRows(unittest.TestCase):

    def test_rows_match_agreed_set(self):
        self.assertEqual(
            build_population_matrices.ROWS,
            (
                "max_national_focus",
                "pop_growth",
            ),
    ...
    def test_filename(self):
        self.assertEqual(
            build_population_matrices.FILENAME, "population_modifiers.csv"
        )


class TestPopulationMain(unittest.TestCase):

    def run_main(self, root: Path, *extra: str) -> Path:
        game_dir = make_category_game_dir(root)
        out = root / "out"
        out.mkdir()
        return build_population_matrices.main(
    ...
    def test_writes_expected_files(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            expected = {
                Path("tech_modifiers/army/population_modifiers.csv"),
                Path("invention_modifiers/army/population_modifiers.csv"),
                Path("westernisation_modifiers/economic/population_modifiers.csv"),
                Path("westernisation_modifiers/military/population_modifiers.csv"),
            }
            self.assertEqual(
                {p.relative_to(out) for p in out.rglob("*.csv")}, expected
            )
            self.assertEqual(
                (out / "tech_modifiers/army/population_modifiers.csv").read_text(
                    encoding="utf-8"
                ),
    ...
    def test_vanilla_anchors_reject_fixture(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with self.assertRaises(ValueError):
                build_population_matrices.main(
                    ["--game-dir", str(game_dir), "--output-dir", str(out)]
                )
