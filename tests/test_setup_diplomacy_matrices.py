"""
Tests for the diplomacy matrices setup (setup/research_modifiers/build_diplomacy_matrices.py).
...
from setup.research_modifiers import build_diplomacy_matrices
from helpers import CAT_SAVE, make_category_game_dir


class TestDiplomacyRows(unittest.TestCase):

    def test_rows_match_agreed_set(self):
        self.assertEqual(
            build_diplomacy_matrices.ROWS,
            (
                "cb_creation_speed",
                "diplomatic_points",
                "influence",
    ...
    def test_filename(self):
        self.assertEqual(
            build_diplomacy_matrices.FILENAME, "diplomacy_modifiers.csv"
        )


class TestDiplomacyMain(unittest.TestCase):

    def run_main(self, root: Path, *extra: str) -> Path:
        game_dir = make_category_game_dir(root)
        out = root / "out"
        out.mkdir()
        return build_diplomacy_matrices.main(
    ...
    def test_writes_expected_files(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            expected = {
                Path("tech_modifiers/army/diplomacy_modifiers.csv"),
                Path("invention_modifiers/army/diplomacy_modifiers.csv"),
                Path("westernisation_modifiers/economic/diplomacy_modifiers.csv"),
                Path("westernisation_modifiers/military/diplomacy_modifiers.csv"),
            }
            self.assertEqual(
                {p.relative_to(out) for p in out.rglob("*.csv")}, expected
            )
            self.assertEqual(
                (out / "tech_modifiers/army/diplomacy_modifiers.csv").read_text(
                    encoding="utf-8"
                ),
                "modifier,col_tech,plain_tech\n"
                "cb_creation_speed,0,0\n"
                "diplomatic_points,0,0\n"
                "influence,0.1,0\n",
            )
            self.assertEqual(
                (
                    out
                    / "invention_modifiers/army/diplomacy_modifiers.csv"
                ).read_text(encoding="utf-8"),
                "modifier,col_invention,effectless_invention\n"
                "cb_creation_speed,0,0\n"
                "diplomatic_points,0.25,0\n"
                "influence,0,0\n",
            )
    ...
    def test_westernisation_files_hold_only_zeros(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(
                (
                    out
                    / "westernisation_modifiers/economic/diplomacy_modifiers.csv"
                ).read_text(encoding="utf-8"),
            ...
            self.assertEqual(
                (
                    out
                    / "westernisation_modifiers/military/diplomacy_modifiers.csv"
                ).read_text(encoding="utf-8"),
