"""
Tests for the colonial matrices setup (setup/build_colonial_matrices.py).
...
from setup import build_colonial_matrices
from helpers import CAT_SAVE, make_category_game_dir


class TestColonialRows(unittest.TestCase):

    def test_rows_match_agreed_set(self):
        self.assertEqual(
            build_colonial_matrices.ROWS,
            (
    ...
    def test_filename(self):
        self.assertEqual(
            build_colonial_matrices.FILENAME, "colonial_modifiers.csv"
        )


class TestColonialMain(unittest.TestCase):

    def run_main(self, root: Path, *extra: str) -> Path:
        game_dir = make_category_game_dir(root)
        out = root / "out"
        out.mkdir()
        return build_colonial_matrices.main(
    ...
    def test_writes_expected_files(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            expected = {
                Path("tech_modifiers/army/colonial_modifiers.csv"),
                Path("invention_modifiers/army/colonial_modifiers.csv"),
                Path("westernisation_modifiers/economic/colonial_modifiers.csv"),
                Path("westernisation_modifiers/military/colonial_modifiers.csv"),
            }
            self.assertEqual(
                {p.relative_to(out) for p in out.rglob("*.csv")}, expected
            )
            self.assertEqual(
                (out / "tech_modifiers/army/colonial_modifiers.csv").read_text(
                    encoding="utf-8"
                ),
                "modifier,col_tech,plain_tech\n"
                "colonial_life_rating,0,0\n"
                "colonial_migration,0,0\n"
                "colonial_points,100,0\n"
                "colonial_prestige,0,0\n",
            )
            self.assertEqual(
                (
                    out
                    / "invention_modifiers/army/colonial_modifiers.csv"
                ).read_text(encoding="utf-8"),
                "modifier,col_invention,effectless_invention\n"
                "colonial_life_rating,0,0\n"
                "colonial_migration,0,0\n"
                "colonial_points,0,0\n"
                "colonial_prestige,0.1,0\n",
            )
    ...
    def test_westernisation_files_hold_only_zeros(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(
                (
                    out
                    / "westernisation_modifiers/military/colonial_modifiers.csv"
                ).read_text(encoding="utf-8"),
