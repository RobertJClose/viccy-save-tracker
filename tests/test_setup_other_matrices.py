"""
Tests for the other-modifier matrices setup (setup/research_modifiers/build_other_matrices.py).
...
from setup.research_modifiers import build_other_matrices
...
class TestOtherRows(unittest.TestCase):
    ...
    def test_rows_match_agreed_set(self):
        self.assertEqual(build_other_matrices.ROWS, ("unit",))

    def test_filename(self):
        self.assertEqual(
            build_other_matrices.FILENAME, "other_modifiers.csv"
        )


class TestOtherMain(unittest.TestCase):

    def run_main(self, root: Path, *extra: str) -> Path:
        game_dir = make_category_game_dir(root)
        out = root / "out"
        out.mkdir()
        return build_other_matrices.main(
    ...
    def test_writes_expected_files(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            expected = {
                Path("tech_modifiers/army/other_modifiers.csv"),
                Path("invention_modifiers/army/other_modifiers.csv"),
                Path("westernisation_modifiers/economic/other_modifiers.csv"),
                Path("westernisation_modifiers/military/other_modifiers.csv"),
            }
            self.assertEqual(
                {p.relative_to(out) for p in out.rglob("*.csv")}, expected
            )
            self.assertEqual(
                (out / "tech_modifiers/army/other_modifiers.csv").read_text(
                    encoding="utf-8"
                ),
