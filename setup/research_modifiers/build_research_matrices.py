"""One-shot initialisation: build the research modifier matrices.

One ``research_modifiers.csv`` per group directory (alongside the
per-goods matrices): rows are the six agreed research names,
...
FILENAME = "research_modifiers.csv"
...
ANCHORS = (
    ...
    ("technology", "culture", "malthusian_thought", "education_efficiency", 0.1),
    ("invention", "culture", "darwinism", "education_efficiency", 0.5),
    (
        "invention",
        "culture",
...
def main(argv: list[str] | None = None) -> Path:
    parser = argparse.ArgumentParser(
        description="Build the research modifier matrices.",
    )
    ...
    print(f"Wrote {len(written)} research matrices to {args.output_dir}")
